...
/**
 * Tearing an application out of the shell and into its own native window.
 *
...
export class DesktopAppWindowBridgeService {
  ...
  async openApp(request: AppWindowRequest): Promise<boolean> {
    if (!request.app) {
      this._lastError.set('openApp: no application named');
      return false;
    }
    if (!this.available()) {
      this._lastError.set('openApp: no native window host');
      return false;
    }

    try {
      await this.surface.call(APP_WINDOW_METHODS.openApp, [asRequestPayload(request)]);
      this._lastError.set('');
      return true;
    } catch (error) {
