 ...
 *  ("social.update.conflict") that pkg/marketing/social.writePost
 *  emits via paths.NewConflictEnvelope on optimistic-lock loss
 *  (Cascade W2, RFC §B.3 row 5). Today only the listener side is
 *  load-bearing — social.ts has no write call-site (Create + MarkSent
 *  are invoked from elsewhere). When an in-view Edit UI lands it MUST
 *  call handleStaleVersionConflict(result, SOCIAL_SERVICE) on its
 *  update path. */
const SOCIAL_SERVICE = "social.update";
...
class LthnViewSocial extends LitElement {
  ...
  /** Cascade W2 (RFC §B.3 row 5) — listener for CONFLICT_RELOAD_EVENT
   *  dispatched by the shared <lthn-conflict-toast>. Scope-filter on
   *  SOCIAL_SERVICE so sibling marketing views' reload signals don't
   *  trigger our refetch. */
  private _onConflictReload = (e: Event) => {
    const ce = e as CustomEvent<ConflictDetail>;
    if (ce.detail?.service !== SOCIAL_SERVICE) return;
  ...
  async _loadFromBackend(): Promise<void> {
    ...
    try {
      ...
      const posts = clampRows<SocialPost>(r?.Value?.posts);
      if (posts.length > 0) this.allPosts = posts;
    } catch {
