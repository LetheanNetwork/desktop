 ...
 *  ("audience.update.conflict") that pkg/marketing/audience.writeSegment
 *  emits via paths.NewConflictEnvelope on optimistic-lock loss
 *  (Cascade W2, RFC §B.3 row 6). Today only the listener side is
 *  load-bearing — audience.ts has no write call-site (Create + Update
 *  are invoked from elsewhere). When an in-view Edit UI lands it MUST
 *  call handleStaleVersionConflict(result, AUDIENCE_SERVICE) on its
 *  update path. */
const AUDIENCE_SERVICE = "audience.update";
...
class LthnViewAudience extends LitElement {
  ...
  /** Cascade W2 (RFC §B.3 row 6) — listener for CONFLICT_RELOAD_EVENT
   *  dispatched by the shared <lthn-conflict-toast>. Scope-filter on
   *  AUDIENCE_SERVICE so sibling marketing views' reload signals don't
   *  trigger our refetch. */
  private _onConflictReload = (e: Event) => {
    const ce = e as CustomEvent<ConflictDetail>;
    if (ce.detail?.service !== AUDIENCE_SERVICE) return;
  ...
  async _loadFromBackend(): Promise<void> {
    ...
    try {
      ...
      const segments = clampRows<Segment>(r?.Value?.segments);
      if (segments.length > 0) this.segments = segments;
    } catch {
