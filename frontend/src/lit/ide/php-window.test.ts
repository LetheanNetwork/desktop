...
//
// Render-smoke test for lthn-php-window. See build-window.test.ts for
...
describe("lthn-php-window — render smoke", () => {
  it("mounts and renders the chrome titlebar without throwing", async () => {
    const { host } = await mountWindow("lthn-php-window");
  ...
  it("collapses to the embedded shell when the embedded attribute is set", async () => {
    const { host } = await mountWindow("lthn-php-window", { attrs: { embedded: "" } });
