/* lthn-core.ts — Lethean core primitives as Lit web components.
 *
...
export type BadgeVariant = "neutral" | "brand" | "success" | "warn";
...
export interface StatusDotProperties {
...
export type StatePillState =
  ...
  | "preview"
  | "latest";
export interface StatePillProperties {
  state?: StatePillState;
}

export type SparklineData = readonly number[];
export interface SparklineProperties {
  data?: SparklineData | string;
  color?: string;
  width?: number;
  height?: number;
  fill?: boolean;
...
export interface BrandMarkProperties {
  size?: BrandMarkSize;
  subdomain?: string;
  name?: string;
}
...
const FA: Record<IconSet, string> = {
  solid: "fas",
  regular: "far",
  light: "fal",
  duotone: "fad",
...
class LthnLight extends LitElement {
  private __slot: string | undefined;
  ...
  override connectedCallback(): void {
    if (this.__slot === undefined) {
      this.__slot = this.innerHTML.trim();
      this.innerHTML = "";
    }
    super.connectedCallback();
  }

  protected slotted(): ReturnType<typeof unsafeHTML> {
    return unsafeHTML(this.__slot || "");
  }
}
...
class LthnIcon extends LthnLight implements IconProperties {
...
class LthnButton extends LthnLight implements ButtonProperties {
  static override properties: PropertyDeclarations = {
    variant: {},
    size: {},
    icon: {},
    iconTrailing: { attribute: "icon-trailing" },
...
class LthnCard extends LthnLight implements CardProperties {
  static override properties: PropertyDeclarations = {
...
class LthnStat extends LthnLight implements StatProperties {
  static override properties: PropertyDeclarations = {
    value: {},
    label: {},
    mono: { type: Boolean },
...
class LthnStatusDot extends LthnLight implements StatusDotProperties {
  static override properties: PropertyDeclarations = {
    variant: {},
    pulse: { type: Boolean },
  };
  ...
  protected override render(): TemplateResult {
    const colors: Record<StatusDotVariant, string> = {
      ...
      err: "var(--danger-400)",
      idle: "var(--fg-3)",
      active: "var(--brand-400)",
    };
    const c = colors[this.variant] || "var(--fg-3)";
...
class LthnStatePill extends LthnLight implements StatePillProperties {
  static override properties: PropertyDeclarations = { state: {} };

  declare state: StatePillState;

  constructor() { super(); this.state = "queued"; }
  protected override render(): TemplateResult {
    const map: Record<StatePillState, readonly [string, string, string]> = {
...
class LthnSparkline extends LthnLight implements SparklineProperties {
  static override properties: PropertyDeclarations = {
    data: {},
    color: {},
    width: { type: Number },
  ...
  declare data: SparklineData | string;
  ...
  protected override render(): TemplateResult {
    ...
    if (s.length < 2) {
      s = Array.from(
        { length: 24 },
        (_value: unknown, i: number): number =>
          30 + Math.sin(i * 0.7) * 10 + Math.sin(i * 1.3) * 5,
      );
    }
    ...
    const pts = s.map(
      (v: number, i: number): string =>
        `${(i / (s.length - 1) * w).toFixed(2)} ${(h - v / m * h).toFixed(2)}`,
    );
...
class LthnDivider extends LthnLight implements DividerProperties {
  static override properties: PropertyDeclarations = {
...
class LthnInput extends LthnLight implements InputProperties {
  static override properties: PropertyDeclarations = {
    type: {},
    placeholder: {},
    value: {},
  ...
  protected override render(): TemplateResult {
    return html`<input
      ...
      @input=${(event: InputEvent): boolean => this.dispatchEvent(new CustomEvent("input", { detail: (event.target as HTMLInputElement).value }))}
...
class LthnField extends LthnLight implements FieldProperties {
  static override properties: PropertyDeclarations = {
    label: {},
    hint: {},
    error: {},
...
/* ───────────── <lthn-toggle on> ───────────── */
class LthnToggle extends LthnLight implements ToggleProperties {
  static override properties: PropertyDeclarations = {
  ...
  protected override render(): TemplateResult {
    ...
    return html`<button role="switch" aria-checked=${on ? "true" : "false"}
      @click=${(): void => { this.on = !this.on; this.dispatchEvent(new CustomEvent("change", { detail: this.on })); }}
...
class LthnBrandMark extends LthnLight implements BrandMarkProperties {
  static override properties: PropertyDeclarations = {
    size: {},
    subdomain: {},
    name: {},
  ...
  declare size: BrandMarkSize;
  declare subdomain: string;
  declare name: string;

  constructor() { super(); this.size = "md"; }
  protected override render(): TemplateResult {
    const sizes: Record<BrandMarkSize, readonly [number, number]> = {
      sm: [14, 6],
      md: [17, 6],
      lg: [22, 8],
    };
    const s = sizes[this.size] || [17, 6];
