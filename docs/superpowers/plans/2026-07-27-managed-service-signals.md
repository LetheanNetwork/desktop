...
- [ ] **Step 1: Write failing Good/Bad/Ugly tests** in `types_test.go` and
...
- [ ] **Step 3: Implement** `Signal`, the four constants, `SignalRequest`, the
...
- [ ] **Step 1: Write failing tests** proving `ProcessRuntime.Signal` and
...
- [ ] **Step 2: Run the focused tests and confirm the red state.**
- [ ] **Step 3: Implement** the interface additions and the named
...
- [ ] **Step 1: Write failing contract tests.** Good: `Kill` on a running
      ...
      restart.
- [ ] **Step 2: Run the focused tests and confirm the red state.**
- [ ] **Step 3: Implement** `Signal` and `Kill` in `lifecycle.go`, emitting the
      existing `services.Event` and the audit trio for both.
- [ ] **Step 4: Run the focused tests and confirm green.**

### Task 4: Expose both through Wails

- [ ] **Step 1: Write failing delegation tests** in `wails_test.go`. Good: both
...
- [ ] **Step 2: Run the focused tests and confirm the red state.**
- [ ] **Step 3: Implement** `Signal(SignalRequest)` and `Kill(id)` in
      `wails.go`, plus the audit names in `pkg/audit/types.go`.
- [ ] **Step 4: Run the focused tests and confirm green.**

### Task 5: Angular bridge

- [ ] **Step 1: Write failing spec tests** for `signal(id, name)` and
...
- [ ] **Step 2: Run the focused specs and confirm the red state.**
- [ ] **Step 3: Implement** both on the bridge with the existing defensive
      parsing.
- [ ] **Step 4: Run the focused specs and confirm green.**

### Task 6: Control surface

- [ ] **Step 1: Write failing presenter specs.** The two actions live behind a
...
- [ ] **Step 2: Run the focused specs and confirm the red state.**
- [ ] **Step 3: Implement** the intents, the presenter control, and the
      orchestration in `control.app.ts`.
- [ ] **Step 4: Run the focused specs and confirm green.**

### Task 7: Verify and record

- [ ] **Step 1: Run the full Go suite** — `GOWORK=$(pwd)/../go.work go test ./...`
      from `go/`. Every package must pass; report the count.
- [ ] **Step 2: Run the full frontend suite** — `npm run test:ci` in
      `frontend-ng/`. Report files and tests passed.
- [ ] **Step 3: Split the `TODO.md` item.** PID and start time are already
...
- [ ] **Step 4: Record the contract in `AGENTS.md`** — named signals only, no
      signal numbers across the boundary, and the restart-policy rule.
- [ ] **Step 5: Tick every box in this plan** that you have completed.
