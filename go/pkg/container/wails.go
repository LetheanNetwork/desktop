...
func (s *Service) List(runtime string) core.Result {
	ctx, cancel := core.WithTimeout(core.Background(), budget(s.budgets.ListAll, defaultListAllBudget))
	defer cancel()
	out := ListOutput{}
...
func (s *Service) Logs(id, runtime string, tail int) core.Result {
	...
	ctx, cancel := core.WithTimeout(core.Background(), budget(s.budgets.Logs, defaultLogsBudget))
	defer cancel()
