...
func TestMediate_Service_Mediate_Good(t *core.T) {
	...
	reply := `{"tool":"engine_rephrase","params":{"text":"please fix this, it's really frustrating","engine_warn_user":true}}`
	...
	pause := w.Mediate(context.Background(), fakeDispatch(`{"tool":"engine_pause","params":{}}`, nil), "anything")
...
func TestMediate_Service_Mediate_Bad(t *core.T) {
	// engine_ok: the engine mis-flagged; the model judges the prompt fine.
	// The model may wrap its JSON in prose — that must still parse, and the
	// reason must survive for the learning corpus.
	w := New(Config{})
	reply := "Sure — here's my call:\n\n{\"tool\":\"engine_ok\",\"params\":{\"reason\":\"'killing' a process is technical, not hostile\"}}\n\nHope that helps."
...
func TestMediate_Service_Mediate_Ugly(t *core.T) {
	...
	// engine_rephrase with empty text is unusable → proceed, learn nothing.
	empty := w.Mediate(ctx, fakeDispatch(`{"tool":"engine_rephrase","params":{"text":"   "}}`, nil), "fix this")
