...
//     sales/contacts + sales/deals + office/mail.AccountProvider;
//     no cached bool, no event bus).
...
//
// AX-8 compliance: this interface is defined in the consumer (pipeline)
// and satisfied by the producer (*account.Service). The duplication
// across sales/contacts, sales/deals, incidents, runbooks, pipeline etc.
// IS the AX-8 boundary — each consumer owns its own contract, no shared
// types package importing producer.
...
//
// Mirrors the sales/contacts + sales/deals + office/mail.AccountProvider
// setter pattern. Live-read on every gate check — no event-bus
// reliability concerns, no cache coherence concerns
...
// hygiene / Cerberus #27 ADD-5). Read-only methods (List) continue to
...
func (s *Service) assertUnlocked(scope string) (core.Result, bool) {
	...
	if g == nil {
		if s.nilWarned.CompareAndSwap(false, true) {
			core.Warn("pipeline: session gate not wired; failing locked", "scope", scope)
		}
		return core.Fail(core.E(scope, "pipeline.session.locked", nil)), false
	}
