...
func cmdHelp(args []string) int {
	if len(args) == 0 {
		...
		core.Println("  opencode <verb>      OpenCode sandbox lifecycle (status / start / stop / providers)")
		core.Println("  marketplace <verb>   Plugin / bundle marketplace (list / install / launch / stop)")
		core.Println("")
	...
	case "ai":
		...
		core.Println("")
		core.Println("Verbs:")
		core.Println("  chat MESSAGE               One-shot user message — prints assistant reply.")
		core.Println("                             Interactive REPL is a window-level concern; run")
		core.Println("                             `lthn gui` and open the chat surface for that.")
		core.Println("  generate PROMPT            One-shot completion.")
		core.Println("  models                     List configured routes (the names you've wired in")
		core.Println("                             ~/Lethean/conf/lthn.yaml). No `ls` / `pull` sub-")
		core.Println("                             verbs today — to download a model, run `lthn gui`")
		core.Println("                             and use the Lemma admin Download form.")
